...
ADE.model <- readRDS(file=paste0(loc.models, "/ADE_model_", dataset.run, "_", nb.models, "_r", ind.run, ".rds"))
specs <- ADE.model$specs
model.init <- ADE.model$model
...
fun_pred <- function(n) {
  ...
  if (n > 1) {
    model.n <- update_ade_meta(model.init, rbind.data.frame(train, test[1:((n-1)*seq.len), ]))
  } else {
    model.n <- model.init
  }
  ...
  for (t in 1:(seq.len-1)) {
    ...
    model.t <- model.n # makes a work copy of the model in the "n-th" state
    
    test.t <- test[ind.seq.n:(ind.seq.n+seq.len-t+1-1), ]
    test.t[2:(seq.len-t+1), target] <- NA
    pred.t <- pred.n[t:seq.len, ]
    
    # forecasts.n <- forecast_tsensembler.rec(test.t, pred.t, seq.len-t+1) # forecasts seq.len-t+1 steps ahead (including current time step)
    h <- seq.len-t+1
    forecasts.n <- rep(NA, h)
    E_hat <- as.data.frame(lapply(model.t@meta_model, function(o) predict(o, test.t)$predictions))
    ...
    for (i in 1:h) {
      ...
      Y_hat_recent <- predict(model.t@base_ensemble, model.t@recent_series, use_all=TRUE)
      Y_rs <- get_y(model.t@recent_series, model.t@form)
      C <- build_committee(Y_hat_recent, Y_rs, model.t@lambda, model.t@omega)
      last_C <- C[[length(C)]]
      Y_hat_j <- Y_hat[1, last_C]
      W_j <- proportion(W[i, last_C])
